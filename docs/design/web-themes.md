...
- 参照基准：Netflix App 2026-04 改版前的三 tab 形态（§2.6）；按本站功能映射为
  ...
  内容首页 `/` 改由顶栏字标直达，底栏让位给高频内容入口；「订阅」对齐桌面
  顶栏的「我的订阅」，按 `canSubscribe` 显隐）。
...
- **「我的」是路由页（`/my`）而非开合面板**（2026-09-14 修订，原右侧滑出的
  ...
  列表、切换账号、退出登录。设置是它的**二级页面**（`/settings/*` 页顶的
  `NetflixSettingsNav` 带返回键：优先消费真实站内来路、直达回落 `/my`）。
...
- 顶栏在 Netflix 移动端**不再放 ☰**（2026-09-14 修订；导航全在底栏页签）：
  字标（回首页）+ 页面级控件 + 搜索；详情页 `PageNav`（返回键 + 吸顶雾）同样
  不放 ☰，保留——App 详情页同样有返回。
